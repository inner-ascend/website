...
const importMetaUrl = {
  ...
  protocol: "http",
  port: 5173
}, serverHost = __SERVER_HOST__, socketProtocol = __HMR_PROTOCOL__ || (importMetaUrl.protocol === "https:" ? "wss" : "ws"), hmrPort = __HMR_PORT__ || 5173;
let rnDevServerHost;
try {
  const { url: devServerUrl } = (0, import_getDevServer.default)(), [, host] = devServerUrl.match(/:\/\/([^\/]+)/) || [];
  host && (rnDevServerHost = host);
} catch (e) {
  console.warn(`[vite-native-client] failed to get react-native dev server url: ${e}`);
}
const hmrHost = rnDevServerHost || `${__HMR_HOSTNAME__ || importMetaUrl.hostname}:${hmrPort || importMetaUrl.port}`, socketHost = `${hmrHost}${__HMR_BASE__}`, directSocketHost = __HMR_DIRECT_TARGET__, base = __BASE__ || "/", messageBuffer = [];
let socket;
try {
  let fallback;
  hmrPort || (fallback = () => {
...
function setupWebSocket(protocol, hostAndPath, onCloseWithoutOpen) {
  const endpoint = `${protocol}://${hostAndPath}`, socket2 = new WebSocket(endpoint, "vite-hmr");
  let isOpened = !1;
  return socket2.addEventListener(
    "open",
    () => {
      isOpened = !0, notifyListeners("vite:ws:connect", { webSocket: socket2 });
    },
    { once: !0 }
  ), socket2.addEventListener("message", ({ data }) => {
  ...
  }), socket2.addEventListener("close", ({ wasClean }) => {
    if (!wasClean) {
      if (!isOpened && onCloseWithoutOpen) {
        onCloseWithoutOpen();
        return;
      }
      notifyListeners("vite:ws:disconnect", { webSocket: socket2 }), console.info("[vite] server connection lost. polling for restart..."), waitForSuccessfulPing(protocol, hostAndPath).then(() => {
...
async function handleMessage(payload) {
  switch (payload.type) {
    ...
    case "update":
      ...
      isFirstUpdate = !1, await Promise.all(
    ...
    case "full-reload":
      if (notifyListeners("vite:beforeFullReload", payload), payload.path && payload.path.endsWith(".html")) {
        const pagePath = decodeURI(location.pathname), payloadPath = base + payload.path.slice(1);
...
async function waitForSuccessfulPing(socketProtocol2, hostAndPath, ms = 1e3) {
  const pingHostProtocol = socketProtocol2 === "wss" ? "https" : "http", ping = async () => {
    try {
...
async function fetchUpdate({
  ...
  const path = pathIn.replace("/@id", ""), acceptedPath = acceptedPathIn.replace("/@id", ""), mod = hotModulesMap.get(path);
  if (!mod) {
  ...
  if (isSelfUpdate || qualifiedCallbacks.length > 0) {
    ...
    try {
      const filePath = acceptedPathWithoutQuery, finalQuery = `file?file=${encodeURIComponent(filePath)}&${explicitImportRequired ? "import&" : ""}t=${timestamp}${query ? `&${query}` : ""}`, scriptUrl = (
      ...
      const source = await fetch(scriptUrl).then((res) => res.text()), evaluatedModule = eval(source);
...
const hotModulesMap = /* @__PURE__ */ new Map(), disposeMap = /* @__PURE__ */ new Map(), pruneMap = /* @__PURE__ */ new Map(), dataMap = /* @__PURE__ */ new Map(), customListenersMap = /* @__PURE__ */ new Map(), ctxToListenersMap = /* @__PURE__ */ new Map();
globalThis.createHotContext = function createHotContext(ownerPath) {
  ...
  if (staleListeners)
    for (const [event, staleFns] of staleListeners) {
      ...
      listeners && customListenersMap.set(
        event,
  ...
  return {
    ...
    invalidate(message) {
      notifyListeners("vite:invalidate", { path: ownerPath, message }), this.send("vite:invalidate", { path: ownerPath, message }), console.info(`[vite] invalidate ${ownerPath}${message ? `: ${message}` : ""}`);
    },
    // custom events
    on(event, cb) {
      const addToMap = (map) => {
    ...
    send(event, data) {
      messageBuffer.push(JSON.stringify({ type: "custom", event, data })), sendMessageBuffer();
    }
  };
};
