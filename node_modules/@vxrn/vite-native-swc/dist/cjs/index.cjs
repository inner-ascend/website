...
const resolve = (0, import_node_module.createRequire)(typeof __filename < "u" ? __filename : import_meta.url).resolve,
  refreshContentRE = /\$Refresh(?:Reg|Sig)\$\(/,
  isWebContainer = globalThis.process?.versions?.webcontainer,
  parsers = {
    ".tsx": {
      syntax: "typescript",
      tsx: !0,
...
var index_default = _options => {
  const hasTransformed = {},
    ...
    transformWithoutGenerators = async (code, id) => {
    ...
    transformWithGenerators = async (code, id) => {
      if (process.env.VXRN_USE_BABEL_FOR_GENERATORS) return await (0, import_transformBabel.transformGenerators)(code);
      const parser = getParser(id);
      return hasTransformed[id] = !0, await (0, import_core.transform)(code, {
        filename: id,
        swcrc: !1,
  ...
  return [{
          ...
          rollupOptions: {
            plugins: [{
              ...
              async transform(code, id) {
                if (asyncGeneratorRegex.test(code)) return await transformWithGenerators(code, id);
                try {
    ...
    async transform(code, _id, transformOptions) {
      if (hasTransformed[_id] || _id.includes("virtual:")) return;
      if (asyncGeneratorRegex.test(code)) return await transformWithGenerators(code, _id);
...
async function swcTransform(_id, code, options) {
  const id = _id.split("?")[0].replace(process.cwd(), ""),
    refresh = options.production || options.noHMR ? !1 : !options.forceJSX,
    result = await transformWithOptions(id, code, options, {
      refresh,
  ...
  if (result) {
    if (!refresh || !refreshContentRE.test(result.code)) return result;
