...
__export(index_exports, {
  ...
  transformCommonJs: () => transformCommonJs,
  transformForBuild: () => transformForBuild,
  transformWithOptions: () => transformWithOptions
});
module.exports = __toCommonJS(index_exports);
var import_core = require("@swc/core"), import_node_module = require("node:module"), import_node_path = require("node:path"), import_transformBabel = require("./transformBabel");
const import_meta = {};
function shouldSourceMap() {
  return process.env.VXRN_ENABLE_SOURCE_MAP === "1";
}
const resolve = (0, import_node_module.createRequire)(
  typeof __filename < "u" ? __filename : import_meta.url
).resolve, refreshContentRE = /\$Refresh(?:Reg|Sig)\$\(/, isWebContainer = globalThis.process?.versions?.webcontainer, parsers = {
...
var index_default = (_options) => {
  const hasTransformed = {}, asyncGeneratorRegex = /(async \*|async function\*|for await)/, transformWithoutGenerators = async (code, id) => {
    const parser = getParser(id);
    return hasTransformed[id] = !0, await (0, import_core.transform)(code, {
      filename: id,
      swcrc: !1,
      configFile: !1,
  ...
  }, transformWithGenerators = async (code, id) => {
    if (process.env.VXRN_USE_BABEL_FOR_GENERATORS)
      return await (0, import_transformBabel.transformGenerators)(code);
    const parser = getParser(id);
    return hasTransformed[id] = !0, await (0, import_core.transform)(code, {
      filename: id,
      swcrc: !1,
  ...
  return [
    {
      ...
      configResolved(config) {
        ...
        isWebContainer && config.logger.warn(
...
async function swcTransform(_id, code, options) {
  const id = _id.split("?")[0].replace(process.cwd(), ""), refresh = options.production || options.noHMR ? !1 : !options.forceJSX, result = await transformWithOptions(id, code, options, {
    refresh,
  ...
  if (result) {
    ...
    if (result.code = wrapSourceInRefreshRuntime(id, result.code, options), result.map) {
      ...
      return sourceMap.mappings = ";;;;;;;;" + sourceMap.mappings, { code: result.code, map: sourceMap };
    }
...
const SHARED_MODULE_CONFIG = {
  importInterop: "none"
  // We want SWC to transform imports to require since there's no Rollup to handle them afterwards, but without adding any interop helpers that would break with our RN module system
}, transformWithOptions = async (id, code, options, reactConfig) => {
  ...
  try {
    ...
    result = await (0, import_core.transform)(code, transformOptions);
  } catch (e) {
    return console.info(
...
const transformCommonJs = async (id, code) => {
  const parser = getParser(id);
  if (parser)
...
}, transformForBuild = async (id, code) => {
  const parser = getParser(id);
  if (parser)
    return await (0, import_core.transform)(code, {
      filename: id,
