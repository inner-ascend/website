  ...
  ({
    ...
    const getColor = (i: number) => {
      ...
      if (mounted !== 'done' || hovered) {
        return isActive ? 'var(--color)' : tints[index % tints.length]
      }
      if (hovered && isActive) {
        return 'var(--color)'
      }
    ...
    const currentIndex = index % tints.length
    const x = Math.round(
      currentIndex * 24.5 + (24 / 2) * (currentIndex / tints.length) + 4 + (currentIndex === 6 ? -4 : 0)
    )
      ...
      >
        ...
        >
          ...
          <path
            ...
            onMouseEnter={() => Tint.setTintIndex(7)}
          />
          {/* N */}
          <path
            data-tauri-drag-region
            ...
            onMouseEnter={() => Tint.setTintIndex(8)}
          />
          {/* N */}
          <path
            data-tauri-drag-region
            ...
            onMouseEnter={() => Tint.setTintIndex(9)}
          />
          {/* I */}
          <path
            data-tauri-drag-region
            ...
            fill={getColor(10)}
            d="M0 40V0H16V40H0Z"
            onMouseEnter={() => Tint.setTintIndex(10)}
          />
        </svg>
      </XStack>
    )
